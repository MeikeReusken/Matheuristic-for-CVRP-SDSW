...
def Math(n, xc, yc, mu, sd, Q, constraint_type, track_data_set, T, a):
    ...
    while Correct_k == 'no':
        ...
        while Condition == 'not satisfied':
            ...
            total_time_clustering = total_time_clustering + (end_time_clustering - start_time_clustering)
            ...
            if Ass_status == 'infeasible' or Ass_status == 'no_sol_in_time':
                # increasing k:
                print('Assignment problem failed - INCREASE K: ', str(k), ' --> ', str(k + 1))
                k = k + 1
                ...
                Routes, Policies, Costs, Duration = Routing(track_data_set, a, p_w, k, Districts, Districts_0,
                                                            xc, yc, mu, Iterations, Summary='yes')
                end_time_routing = time.perf_counter()
                total_time_routing = total_time_routing + (end_time_routing - start_time_routing)

                temporary_df = pd.DataFrame(
                    columns=['iteration', 'driving', 'handling', 'waiting', 'total expected duration', 'policy'])
                temporary_df[['driving', 'handling', 'waiting', 'driving_only', 'recourse']] = Duration
                temporary_df['iteration'] = Iterations
                temporary_df['total expected duration'] = temporary_df['driving'] + temporary_df['handling'] + \
                                                          temporary_df[
                                                              'waiting']
                temporary_df['policy'] = Policies
                Complete_Duration = pd.concat([Complete_Duration, temporary_df])
                ...
                # count policies
                unique_policy, counts_policy = np.unique(Policies, return_counts=True)
                ...
                Correct_k, k, K, deck = Check_number_districts(T, k, K, reason_m, Duration, b, mu, sd, k_tested)
                ...
                if Correct_k == 'yes':
                    ...
                    K_violated_in_iteration, fail_cc, fail_ec, eta_check = Check_Feasibility(T, b, k, Districts_0,
                                                                                             Districts, Duration,
                ...
                Findings = [n, str(k) + reason_m,
                            ...
                            (Clustering_objective + 2 * sum(c[0][N_s_1])) / s,  # A2
                            A1,
                            max(Duration.sum(axis=1)), min(Duration.sum(axis=1)), sum(Duration.sum(axis=1)) / k,
                            end_time - start_time,  # in seconds
                            total_time_clustering,
                            total_time_routing,
                            Clustering_gap,  # in percentages
                            Iterations, Feasibility, Feas_constraint_nd, Feas_constraint_nd_decrease,
                            Feas_constraint_cc, Feas_constraint_ec, E,
                            rho_round, Ending_reason, total_driving_only, total_recourse, total_waiting, total_handling,
                            total_all, total_preventive, total_risk]

                # store uncertainty results
                gamma_test = 1 - st.norm.cdf((k * Q - sum(mu)) / np.sqrt(sum(sd ** 2)))
                delta_test = 1 - st.norm.cdf(
                    (k * T - sum(Duration['driving']) - sum(b * mu)) / np.sqrt(sum((np.array(b) ** 2) * (sd ** 2))))
                alpha_Z = [(Q - sum(mu[Districts_0[i]])) / np.sqrt(sum(sd[Districts_0[i]] ** 2)) for i in range(k)]
                alpha_test = 1 - st.norm.cdf(alpha_Z)
                alpha_test_av = np.nanmean(alpha_test)
                eta_test = eta_check # already derived earlier in the matheuristic
